...
def mean_metric(dataset: str, bucket: str, method: str, metric: str) -> float:
    data = load_json(f"results/{dataset}_vara_results.json")
...
def care_vs_mamex_zero_shot() -> tuple[str, str]:
    care_baby = load_json("results/baby_vara_results.json")["per_seed"][0]
    care_office = load_json("results/office_vara_results.json")["per_seed"][0]
...
def yelp_cross_domain_dashboard() -> tuple[str, str]:
    seed = load_json("results/yelp_vara_results.json")["per_seed"][0]
    labels = ["1--4", ">20"]
...
def full_ranking_consistency() -> tuple[str, str]:
    sampled = load_json("results/baby_vara_results.json")["per_seed"][0]
