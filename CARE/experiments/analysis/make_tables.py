...
功能：
  ...
  2. 合并 MARA 主实验结果（从 checkpoints/train_summary.json）
...
def main():
    ...
    # MARA 主结果（从多个 seed 的 train_summary 读取）
    mara_test_ndcg10 = []
    mara_test_hr10   = []
    for seed in [42, 123, 456]:
        ...
        if p.exists():
            ...
            mara_test_ndcg10.append(s.get("best_ndcg10", 0.0))
    ...
    # MARA 行
    if mara_test_ndcg10:
        mean_n = np.mean(mara_test_ndcg10)
        std_n  = np.std(mara_test_ndcg10)
        print(f"  {'MARA (ours)':<14} {'—':>10} {mean_n:>10.4f}±{std_n:.4f} "
              f"{'—':>10} {'—':>10} {'—':>10} {'—':>10}")
    ...
    if not args.no_complexity:
        ...
        try:
            print("\n  正在测量 MARA 复杂度...")
            mara_c = measure_complexity(ds)
            print(f"  {'MARA':<12} {mara_c['params_M']:>10.2f} "
                  f"{mara_c['mem_GB']:>10.2f} {mara_c['latency_ms']:>15.1f}")
        except Exception as e:
            print(f"  MARA 复杂度测量失败: {e}")
