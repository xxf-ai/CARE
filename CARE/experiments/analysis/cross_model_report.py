...
def load_vara_stratified(dataset: str) -> dict:
    """Load VARA main results, extract stratified data aggregated across seeds.

    Returns: {bucket: {strategy: {ndcg@10, hr@10, n}}}
    """
    path = VARA_RESULT_DIR / f"{dataset}_vara_results.json"
    if not path.exists():
        print(f"  [WARN] 未找到: {path}")
        return {}
...
def collect_all(datasets: list[str]) -> dict:
    ...
    for ds in datasets:
        # Baseline models from stratified JSON (BPR row already sourced from VARA results)
        merged = load_stratified_results(ds)
        if not merged:
            merged = {}

        # If no stratified JSON exists yet, directly load VARA results as BPR fallback
        if "BPR" not in merged:
            vara = load_vara_stratified(ds)
            if vara:
                merged["BPR"] = vara
