...
def _load_vara_stratified(dataset, seeds, alpha):
    """BPR 行直接复用 VARA 主实验结果中的分层数据，保证与论文 Table 3 一致。"""
    vara_path = ROOT / "results" / f"{dataset}_vara_results.json"
    if not vara_path.exists():
        print(f"  [WARN] VARA 结果文件不存在: {vara_path}, 回退到 checkpoint 评估")
        return None
    ...
    all_results = {}
    json_seeds = vara.get("seeds", seeds)
    for i, per_seed in enumerate(vara["per_seed"]):
        ...
        all_results[seed] = {"result": result, "alpha": alpha, "ckpt": "VARA (from paper)"}
    return all_results
...
def main():
    ...
    if args.alpha is None:
        try:
            vara_results = json.load(
                open(ROOT / "results" / f"{args.dataset}_vara_results.json"))
            best_alpha = vara_results["per_seed"][0].get("gate_best_alpha", 1.0)
        except Exception:
    ...
    for model_name in args.models:
        ...
        # BPR 行直接复用 VARA 主实验分层数据，保证与论文 Table 3 一致
        if model_name == "BPR":
            bpr_results = _load_vara_stratified(args.dataset, args.seeds, alphas_to_try[0])
            if bpr_results:
