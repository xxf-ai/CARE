...
def run_stratified_for_model(model_name, data_dir, cache_dir, device, seeds, dataset, alpha):
    """加载已训练的 checkpoint 并跑分层评估。BPR 复用 VARA 原始数据保证一致性。"""
    from experiments.baselines.run_stratified_baselines import evaluate_one_baseline, _load_vara_stratified

    # BPR 行直接复用 VARA 主实验分层数据
    if model_name == "BPR":
        bpr = _load_vara_stratified(dataset, seeds, alpha)
        if bpr:
...
def main():
    ...
    if not args.skip_train:
        for model_name in args.models:
            ...
            if model_name == "BPR":
                print(f"\n  [SKIP] BPR 不需训练，分层评估直接复用 VARA 数据")
                continue
