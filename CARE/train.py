"""
train.py — CARE training entry point
...
def train(args, cfg=None):
    ...
    print(f"\n{'='*60}")
    print(f"  CARE 训练  数据集={args.dataset}  seed={args.seed}")
    ...
    # ── Model ─────────────────────────────────────────────────────
    model = CARE(n_users, n_items, run_cfg).to(device)
